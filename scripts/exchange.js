...
async function main() {
	...
	const proposed_quality =
		BigNumber(we_spend.value) / BigNumber(we_want.value);
	...
	if (!offers) {
		...
		for (const o of offers) {
            ...
            ${we_want.currency}`);
				running_total = running_total.plus(
					BigNumber(o.owner_funds)
				);
		...
		if (running_total > 0 && running_total < want_amt) {
			console.log(`Remaining ${want_amt - running_total} ${
				we_want.currency
			}
		...
		if (!offers2) {
			...
			for (const o of offers2) {
                ...
                ${o.owner_funds} ${tally_currency}`);
					running_total2 = running_total2.plus(
						BigNumber(o.owner_funds)
					);
				} else {
					console.log(
